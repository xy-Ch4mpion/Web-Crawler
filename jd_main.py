...
result =pd.DataFrame(columns=('商品ID','商品品牌','商品名称','商品产地','商品毛重','评论数','好评率','价格'))
...
def location():
    ...
    for id in m_id:
        url = 'https://item.jd.com/%s.html'%(id)
        result_location = requests.get(url).text
        r_location = r"<li title='(.*?)'>商品产地"
        m_location = re.findall(r_location, result_location)
        if m_location:
            location_list.append(m_location[0])
        else:
...
#        driverr = webdriver.Chrome(executable_path=path)
#        url = 'https://item.jd.com/%s.html'%(id)
#        driverr.get(url)
#        html = etree.HTML(driverr.page_source)
#        price_html = etree.tostring(html, encoding = "utf-8", pretty_print= True, method = "html")
#        price_html = price_html.decode('utf-8')
#        driverr.close()
#        r_price = r'<span class="price J-p-%s">(.*?)</span>'%(id)
#        m_price = re.findall(r_price, price_html)
#        if m_price:
#            price_list.append(m_price[0])
#        else:
#            price_list.append('NA')
#    print(price_list)

def price():
    global m_price, price_list
    price_list = []
    for id in m_id:
        r_price = r'<strong class="J_%s" data-done="1"><em>￥</em><i>(.*?)</i></strong>'%(id)
...
if __name__=="__main__":
    ...
    for page in range(0, 7):
        ...
        standardize(location_list)
        standardize(price_list)
        result=result.append(pd.DataFrame({'商品ID':m_id,'商品品牌':brands_brands,'商品名称':item_list,'商品产地':location_list,'商品毛重':weight_list,'评论数':rating_countStr,'好评率':rating_list,'价格':price_list}),ignore_index=True)
